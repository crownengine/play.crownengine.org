...
// Begin JS library code
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(69, 0, 1, fd);
...
var ASM_CONSTS = {
  420846: ($0) => { window.open(UTF8ToString($0), "_blank"); },  
 420891: ($0) => { var saveDir = UTF8ToString($0); if (saveDir.charAt(0) != "/") saveDir = "/" + saveDir; Module.CrownSaveGame = Module.CrownSaveGame || {}; var state = Module.CrownSaveGame; if (state.mountDir == saveDir && state.ready) return; state.mountDir = saveDir; state.ready = 0; state.error = 0; state.syncing = 0; state.syncError = 0; function mkdirTree(dir) { var path = ""; var parts = dir.split("/"); for (var ii = 0; ii < parts.length; ++ii) { if (!parts[ii]) continue; path += "/" + parts[ii]; try { FS.mkdir(path); } catch (err) { } } } try { mkdirTree(saveDir); FS.mount(IDBFS, {}, saveDir); FS.syncfs(true, function(err) { state.error = err ? 1 : 0; state.ready = 1; }); } catch (err) { console.error(err); state.error = 1; state.ready = 1; } },  
 421637: () => { return Module.CrownSaveGame && Module.CrownSaveGame.ready ? 1 : 0; },  
 421708: () => { return Module.CrownSaveGame && !Module.CrownSaveGame.error ? 1 : 0; },  
 421780: () => { var state = Module.CrownSaveGame || (Module.CrownSaveGame = {}); state.syncing = 1; state.syncError = 0; try { FS.syncfs(false, function(err) { state.syncError = err ? 1 : 0; state.syncing = 0; }); } catch (err) { console.error(err); state.syncError = 1; state.syncing = 0; } },  
 422060: () => { return Module.CrownSaveGame && Module.CrownSaveGame.syncing ? 1 : 0; },  
 422133: () => { var s = Module.CrownSaveGame; return s && !s.error && !s.syncError ? 1 : 0; },  
 422213: () => { debugger; }
};
